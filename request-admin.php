...
if(isset($_GET['accepted-request'])) {
    $order = $db->query('SELECT * FROM `requests` WHERE accepted = 1 ORDER BY `id_request` DESC LIMIT 9');
} else {
    $order = $db->query('SELECT * FROM `requests` WHERE accepted = 0 ORDER BY `id_request` DESC LIMIT 9');
}

/*---------------------------------------------------------------------------------------------------------*/
// Afficher parties textuelles actuelles de la page principale
...
/* --------------------------------------------------------------------------------------------------------------- */
// Récupération de tous les utilisateurs pour les lister
$reqUser = $db->query("SELECT * FROM `admin`;");
$AllUser = $reqUser->fetchAll(PDO::FETCH_ASSOC);
...
<!---------------------------------------------------------------------------------------------------------------------------------------------------------->
                ...
                <form id="search-request" method="GET" >
                    <input maxlength=20 type="text" name="search" value="<?php echo $search ?>" 
                    placeholder="Rechercher parmis <?php echo($countRequest); ?> demande(s)" />
                    <input id="invisible" type="submit" />
                </form>
                 <!----------------------------------------->
                 <section id="add-new-account">
                    <div class="container">
                        <form action="db-new-user.php" method="POST" class="dflex fdcolumn" enctype="multipart/form-data">
...
<!----------------------------------------------------------------------------------------------------------->         
<?php
foreach($AllUser as $toto) {
?>
<!----------------------------------------------------------------------------------------------------------->   
                <div class="container">
                    <div class="article-card-text">
                        <li>&#128100; <?=$toto['login']?> - <a href="delete-user.php?id=<?=$toto['id_admin']?>">&#10060;</a></li>
                        <li>&#128232; <?=$toto['email']?><li>
                    </div>
                </div>
<!----------------------------------------------------------------------------------------------------------->
...
<?php
    if(count($repSearch) == 0) {
        ...
        while ($newRequest = $order->fetch(PDO::FETCH_ASSOC)) {
?>
<!---------------------------------------------------------------------------------------------------------------------------------------------------------->
